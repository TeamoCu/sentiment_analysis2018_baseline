...
if __name__ == '__main__':
    ...
    # 将要转换的词库添加在这里就可以了
    o = [u'荤，蔬，豆，果类.scel', u'各地糕点名.scel', u'酒类品牌.scel', u'西餐用语.scel',
         u'生鲜餐饮词库.scel', u'八大菜系菜单列表.scel', u'中国南北大菜-- 菜名大全.scel', u'给超爱西餐的你【官方推荐】.scel',
         u'香港小吃【官方推荐】.scel', u'满汉全席【官方推荐】.scel', u'八大菜系【官方推荐】.scel', u'饮食大全【官方推荐】.scel', u'北京话大集合【官方推荐】.scel']

    for f in o:
        deal(f)

        # 保存结果
    f = io.open('amuse.txt', 'w', encoding='utf-8')
    for word in GTable:
        ...
        f.write(u'\n')
    f.close()
    stopwords = [line.strip() for line in io.open('amuse.txt', 'r', encoding='utf-8').readlines()]
    words = list(set(stopwords))
    f = io.open(config.dict_path, 'w', encoding='utf-8')
    for word in words:
        f.write(word)
        f.write(u'\n')
    f.close()
